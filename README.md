# Welcome the containerized services architecture era with Docker-compose !

Your project is organized though different technical services e.g. back-office, front-end application, back-end application, database... You can have 3 services, 8 services or even more: perhaps every one of your domain's services is related to a technical service - which means a dozen of services for your project.
...
**Docker** is you best friend in this achievement.
...
> You can follow this tutorial with this application example available [here](https://github.com/tpucci/react-bucket-list/tree/docker/containerless).
...
- the backend should be accessed with the same root URL than our frontend : the API is our **second service** and will be discovered behind a proxy of our first server.
- the database should be accessed with a URL and some creditentials.

In order to achieve our goal (which is to make each of our services manageable), we can use **Docker** to containerize them. The target container architecture is given here:
...
## Draw your development architecture
...
Instead of building the entire architecture each time we make a change in our code, we would still use our development servers. The following diagram shows the development architecture (differences with target architecture are showed in green):
...
Drawing our architecture is an important step to achieve our goal! You'll see it makes our Docker container easier to write.

The following steps will lead you to execute the development environement on your machine.

### 1. Create the `docker` directory, and one Dockerfile per development service.
...
The content of `nginx.dev.conf` will not be described here as NGINX is not the purpose of this tutorial. However, you can have a look at my configuration file [here](https://github.com/tpucci/react-bucket-list/blob/docker/containerfull/config/nginx/nginx.dev.conf). It should declare the two `app` and `api` upstreams.
...
- ir should expose its `3000` port (Webpack Dev Server port).
...
> In our application example, you should also change the `API_URL` to `http://localhost/api` in `app/src/App.js`:
>
> `6: const API_URL = 'http://localhost/api';`
...
### 2. Create the `docker/build.yml` file and declare our services.
...
Docker Compose launched every container in accordingly to our configuration file. Once launched, every service print their logs in **one single terminal**. We made every service resource manageable and furthermore, we still use our efficient development tools !
